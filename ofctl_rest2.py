...
from controlador import retornaLinks 
...
class RestStatsApi(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        mapper.connect('stats', uri,
                       controller=StatsController, action='teste',
                       conditions=dict(method=['GET']))


        uri = path3+'/getLinks'
        mapper.connect('link', uri,
                               controller=StatsController, action='get_Links',
                               conditions=dict(method=['GET']))

        uri = path2 + '/infointerface/{eth}'
        mapper.connect('itfcs', uri,
                       controller=StatsController, action='get_InfoInterface',
                       conditions=dict(method=['GET']))
